...
also use admitted async reads. Packed scalar V3 keeps its existing reader and
materializer.
...
The producer acquires leases before dispatching child coroutines. Raw ranges
are flattened across file boundaries. All consumers place disjoint ranges by
offset; file consumers await `PositionedFileWriter::WriteAt` on `LocalFileIOPool`.
There is no ordered-consumer mode or preceding-slice wait. `WriteAt` and `Finish`
share the existing global write permits with `FileWriter`; permits cover blocking
local I/O and never span remote reads or coroutine waits.

The window advances when any unit completes, using the same completion-queue
pattern as `IndexMaterializer`. A slow first unit does not block refill when
other units finish. Read/decode buffers are destroyed and admission released
before completion is published. The loader retains only bounded completion
...
assertions enforce valid, 4 KiB-aligned slice sizes and a nonzero dispatch window.
...
consumer starts. Knowhere mmap write histograms observe each actual local write
and the final close phase, excluding executor queueing. These are per-operation
