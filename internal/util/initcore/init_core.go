...
func SetupCoreConfigChangelCallback() {
	coreParamCallbackInitOnce.Do(func() {
		...
		paramtable.Get().CommonCfg.ScalarIndexLoadMemoryLimitMB.RegisterCallback(func(ctx context.Context, key, oldValue, newValue string) error {
			size, err := strconv.Atoi(newValue)
			if err != nil {
				return err
			}
			UpdateScalarIndexLoadMemoryLimitMB(size)
			return nil
		})

		paramtable.Get().CommonCfg.ScalarIndexLoadStreamChunkSizeMB.RegisterCallback(func(ctx context.Context, key, oldValue, newValue string) error {
			size, err := strconv.Atoi(newValue)
			if err != nil {
				return err
			}
			UpdateScalarIndexLoadStreamChunkSizeMB(size)
			return nil
		})
