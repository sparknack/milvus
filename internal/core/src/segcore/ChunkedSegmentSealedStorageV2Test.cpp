...
class TestChunkSegmentStorageV2 : public testing::TestWithParam<bool> {
 protected:
    ...
    CreateSegment(bool is_sorted_by_pk) {
        auto seg = segcore::CreateSealedSegment(schema_,
                                                nullptr,
                                                -1,
...
TEST_P(TestChunkSegmentStorageV2, TestLazySystemIndexesOnUnsortedSegment) {
    ...
    segment_internal->mask_with_delete(delete_mask_view,
                                       chunk_num * test_data_count,
                                       MAX_TIMESTAMP);
...
TEST_P(TestChunkSegmentStorageV2, TestLazySystemIndexesOnSortedSegment) {
    ...
    ASSERT_EQ(chunk_num * test_data_count - 1, sorted_segment->get_real_count());

    if (!GetParam()) {
        int64_t seg_offsets[] = {0, 42};
        auto pk_result =
            sorted_segment->bulk_subscript(nullptr, fields.at("pk"), seg_offsets, 2);
