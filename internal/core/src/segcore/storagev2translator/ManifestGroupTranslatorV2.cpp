...
ManifestGroupTranslatorV2::ManifestGroupTranslatorV2(
    int64_t segment_id,
...
size_t
ManifestGroupTranslatorV2::num_cells() const {
    return meta_.chunk_memory_size_.size();
}

milvus::cachinglayer::cid_t
ManifestGroupTranslatorV2::cell_id_of(milvus::cachinglayer::uid_t uid) const {
    return uid;
}

std::pair<milvus::cachinglayer::ResourceUsage,
          milvus::cachinglayer::ResourceUsage>
ManifestGroupTranslatorV2::estimated_byte_size_of_cell(
...
const std::string&
ManifestGroupTranslatorV2::key() const {
    return key_;
}

std::vector<
    std::pair<milvus::cachinglayer::cid_t, std::unique_ptr<milvus::GroupChunk>>>
ManifestGroupTranslatorV2::get_cells(
    ...
    auto load_units = BuildStorageV3LoadUnitsForCells(
...
std::unique_ptr<milvus::GroupChunk>
ManifestGroupTranslatorV2::load_group_chunk(
...
int64_t
ManifestGroupTranslatorV2::loading_overhead_bytes(int64_t cell_size) const {
    if (!has_array_field_) {
        return cell_size;
    }
