...
// Replaces [\s\S] with a byte-level pattern matching one complete UTF-8
// character sequence.
inline std::string
...
TEST(InvalidUTF8ConsistencyTest, TruncatedMultibyteSequence) {
    ...
    // Test with underscore wildcard
    {
        ...
        // Both reject: RE2 rejects invalid UTF-8, LikePatternMatcher also
        // rejects since _ only matches valid UTF-8 characters.
        EXPECT_FALSE(re2_result);
        EXPECT_FALSE(like_result);
        EXPECT_EQ(re2_result, like_result)
            << "RE2/Like must agree on truncated 2-byte UTF-8 handling";
    }

    // With percent wildcard: LikePatternMatcher matches (% accepts any
    // trailing bytes without validating UTF-8), while RE2 in UTF-8 mode
    // rejects the entire string. This divergence is acceptable: % means
    // "match anything" and validating encoding on the % path would add
    // overhead for a case that never occurs (Go layer rejects invalid UTF-8).
    {
        std::string pattern = "caf%";
        LikePatternMatcher like_matcher(pattern);
        EXPECT_TRUE(like_matcher(truncated_2byte));
    }
}
...
TEST(InvalidUTF8ConsistencyTest, OverlongEncoding) {
    ...
    // This is invalid UTF-8 - both matchers should reject
    ...
    // Both RE2 (UTF-8 mode) and LikePatternMatcher reject invalid UTF-8
    // for _ wildcard: RE2's . won't match invalid bytes,
    // Utf8WildcardCharByteLen returns 0 for overlong sequences (0xC1 < 0xC2).
    EXPECT_EQ(re2_result, like_result)
        << "RE2/Like must agree on overlong UTF-8 encoding";
}
...
TEST(InvalidUTF8ConsistencyTest, InvalidContinuationByte) {
    ...
    // Both reject: RE2's [\s\S] won't match lone continuation byte,
    // Utf8WildcardCharByteLen returns 0 for invalid bytes.
    EXPECT_FALSE(re2_result);
    EXPECT_FALSE(like_result);
    EXPECT_EQ(re2_result, like_result)
...
// so embedded NUL bytes are compared correctly.

TEST(NULAwareMatchTest, PrefixMatchWithEmbeddedNUL) {
    using namespace milvus;

    // Strings that share a prefix up to and including a NUL byte,
    // but differ after the NUL.
    std::string data("hello\0world", 11);     // "hello\0world"
    std::string prefix1("hello\0world", 11);  // exact match
    std::string prefix2("hello\0wrong", 11);  // differs after NUL
    std::string prefix3("hello\0", 6);        // prefix up to NUL

    EXPECT_TRUE(PrefixMatch(data, prefix1));
    EXPECT_FALSE(PrefixMatch(data, prefix2))
        << "Must compare bytes after NUL — strncmp would wrongly return true";
    EXPECT_TRUE(PrefixMatch(data, prefix3));

    // Single NUL byte as prefix
    std::string nul_data("\0abc", 4);
    std::string nul_prefix("\0", 1);
    std::string wrong_prefix("a", 1);
    EXPECT_TRUE(PrefixMatch(nul_data, nul_prefix));
    EXPECT_FALSE(PrefixMatch(nul_data, wrong_prefix));
}

TEST(NULAwareMatchTest, PostfixMatchWithEmbeddedNUL) {
    using namespace milvus;

    std::string data("hello\0world", 11);
    std::string postfix1("\0world", 6);  // correct suffix
    std::string postfix2("\0wrong", 6);  // wrong suffix after NUL

    EXPECT_TRUE(PostfixMatch(data, postfix1));
    EXPECT_FALSE(PostfixMatch(data, postfix2))
        << "Must compare bytes after NUL — strncmp would wrongly return true";

    // Trailing NUL
    std::string data2("abc\0", 4);
    std::string postfix3("c\0", 2);
    std::string postfix4("c", 1);
    EXPECT_TRUE(PostfixMatch(data2, postfix3));
    EXPECT_FALSE(PostfixMatch(data2, postfix4));
}

TEST(NULAwareMatchTest, InnerMatchWithEmbeddedNUL) {
    using namespace milvus;

    std::string data("aa\0bb\0cc", 8);
    std::string pattern1("\0bb\0", 4);  // substring with NULs
    std::string pattern2("\0bc\0", 4);  // not a substring

    EXPECT_TRUE(InnerMatch(data, pattern1));
    EXPECT_FALSE(InnerMatch(data, pattern2));
}

TEST(NULAwareMatchTest, LikePatternMatcherWithEmbeddedNUL) {
    using namespace milvus;

    // LIKE 'hello\x00%' should match "hello\x00world" but not "helloxworld"
    std::string pattern("hello\0%", 7);
    LikePatternMatcher matcher(pattern);

    std::string data1("hello\0world", 11);
    std::string data2("hello\0", 6);
    std::string data3("helloxworld", 11);

    EXPECT_TRUE(matcher(data1));
    EXPECT_TRUE(matcher(data2));
    EXPECT_FALSE(matcher(data3));
}
...
// Regression test: FindSegment must not return a match that starts inside a
// multi-byte UTF-8 character. str.find() does byte-level matching so it can
// land on a continuation byte; the matcher must skip such false hits.
TEST(UTF8ConsistencyTest, FindSegmentMidCharacterBoundary) {
    using namespace milvus;

    // Construct a string where the literal bytes of the search segment
    // happen to appear as a *suffix* of a multi-byte character followed
    // by the next character.
    //
    // UTF-8 for U+00E9 (é) is 0xC3 0xA9.
    // If we search for the ASCII byte 0xA9 (©) via pattern "%\xA9%",
    // str.find() might match the 0xA9 continuation byte inside é.
    // The matcher must reject that position because it is mid-character.

    // String: "é" (C3 A9) + "test"
    std::string str_with_e_acute = "\xC3\xA9test";

    // Pattern: %©%  where © is 0xC2 0xA9 in UTF-8
    // This should NOT match because é's second byte 0xA9 is a continuation
    // byte, not a standalone ©.
    {
        std::string pattern = "%\xC2\xA9%";  // %©%
        PatternMatchTranslator translator;
        auto regex_pattern = translator(pattern);
        RegexMatcher re2_matcher(regex_pattern);
        LikePatternMatcher like_matcher(pattern);

        bool re2_result = re2_matcher(str_with_e_acute);
        bool like_result = like_matcher(str_with_e_acute);
        EXPECT_EQ(re2_result, like_result)
            << "FindSegment mid-character boundary: é string with ©  pattern";
    }

    // String: "\xE4\xBD\xA0" (你, 3-byte) + "\xA0test"
    // The byte 0xA0 appears both as continuation byte in 你 and as first
    // byte of the next sequence. Pattern searching for 0xA0 + "test" must
    // not accidentally match starting at the continuation byte inside 你.
    {
        // 你 = E4 BD A0, then \xC2\xA0 (non-breaking space) + "est"
        std::string str =
            "\xE4\xBD\xA0\xC2\xA0"
            "est";
        // Search for "\xBD\xA0" which appears across the 你 boundary.
        // Note: \xBD\xA0 are raw continuation bytes (invalid UTF-8),
        // so RE2 in UTF-8 mode correctly refuses to compile this pattern.
        // Only test with LikePatternMatcher here.
        std::string pattern = "%\xBD\xA0%";
        LikePatternMatcher like_matcher(pattern);
        bool like_result = like_matcher(str);
        // LikePatternMatcher does byte-level matching for literals,
        // so it may or may not match depending on implementation.
        // The key point: it should not crash.
        (void)like_result;
    }

    // Positive case: the literal does appear at a valid character boundary
    {
        // String: "abc" + "©" (C2 A9) + "def"
        std::string str =
            "abc\xC2\xA9"
            "def";
        std::string pattern = "%\xC2\xA9%";  // %©%
        ...
        bool like_result = like_matcher(str);
        EXPECT_EQ(re2_result, like_result)
            << "FindSegment: valid boundary match with ©";
        EXPECT_TRUE(like_result)
            << "Should match when literal is at a valid character boundary";
    }
}

// ============== Pattern Matching Performance Benchmark ==============
// Compares throughput of: RE2, Boost regex, and LikePatternMatcher
// across different pattern types and data sizes.
// Run with: --gtest_filter="*PatternMatchBenchmark*"

namespace {

// Generate random strings of given average length
std::vector<std::string>
GenerateRandomStrings(size_t count, size_t avg_len, unsigned seed = 42) {
...
struct BenchResult {
    std::string name;
    double us_per_match;  // microseconds per match operation
    int64_t matches;      // number of matches found
};
...
RunBench(const std::string& name,
         ...
         MatcherFn&& matcher_fn,
         int warmup_iters = 2,
         int bench_iters = 5) {
    volatile int64_t matches = 0;

    // Warmup
    for (int i = 0; i < warmup_iters; i++) {
        for (const auto& s : data) {
            matches += matcher_fn(s) ? 1 : 0;
        }
    }
    ...
    double us_per_match = total_us / (data.size() * bench_iters);

    return {name, us_per_match, matches / bench_iters};
}

void
PrintResults(const std::string& header,
             const std::vector<BenchResult>& results) {
    std::cout << "\n  " << header << "\n";
    std::cout << "  " << std::string(60, '-') << "\n";
    std::cout << std::left << "  " << std::setw(25) << "Matcher" << std::right
              << std::setw(15) << "us/match" << std::setw(12) << "matches"
              << "\n";
    std::cout << "  " << std::string(60, '-') << "\n";
    for (const auto& r : results) {
        std::cout << "  " << std::left << std::setw(25) << r.name << std::right
                  << std::setw(12) << std::fixed << std::setprecision(3)
                  << r.us_per_match << " us" << std::setw(12) << r.matches
                  << "\n";
    }
}
...
TEST(PatternMatchBenchmark, MatcherComparison) {
    using namespace milvus;

    const size_t N = 100000;
    auto data = GenerateRandomStrings(N, 50);
    ...
    std::vector<PatternCase> patterns = {
        {"prefix: abc%", "abc%"},
        {"suffix: %xyz", "%xyz"},
        {"inner: %hello%", "%hello%"},
        {"underscore: a_c_e", "a_c_e"},
        {"complex: %ab%cd%ef%", "%ab%cd%ef%"},
        {"exact: abcdefghij", "abcdefghij"},
    };

    std::cout << "\n====== Pattern Match Benchmark (" << N
              << " strings, avg 50 bytes) ======\n";

    PatternMatchTranslator translator;

    for (const auto& pc : patterns) {
        auto regex_pattern = translator(pc.pattern);

        // Pre-construct matchers (exclude construction time)
        RegexMatcher re2_matcher(regex_pattern);
        BoostRegexMatcher boost_matcher(regex_pattern);
        LikePatternMatcher like_matcher(pc.pattern);

        std::vector<BenchResult> results;

        results.push_back(
            RunBench("RE2 (UTF-8)", data, [&](const std::string& s) {
                return re2_matcher(s);
            }));

        results.push_back(
            RunBench("Boost regex", data, [&](const std::string& s) {
                return boost_matcher(s);
            }));

        results.push_back(
            RunBench("LikePatternMatcher", data, [&](const std::string& s) {
                return like_matcher(s);
            }));

        PrintResults(pc.name, results);
    }
}

TEST(PatternMatchBenchmark, CompilationOverhead) {
    using namespace milvus;
    PatternMatchTranslator translator;
    ...
    std::cout
        << "\n====== Pattern Compilation Benchmark (10000 iterations) ======\n";
    ...
    for (const auto& pattern : patterns) {
        ...
        for (int i = 0; i < iters; i++) {
            BoostRegexMatcher m(regex_pattern);
            (void)m;
        }
        ...
        // LikePatternMatcher construction
        start = std::chrono::high_resolution_clock::now();
        for (int i = 0; i < iters; i++) {
            LikePatternMatcher m(pattern);
            (void)m;
        }
        end = std::chrono::high_resolution_clock::now();
        double like_us =
        ...
        std::cout << "\n  Pattern: \"" << pattern << "\"\n";
        std::cout << "    RE2 compile:        " << std::fixed
                  << std::setprecision(3) << re2_us << " us\n";
        std::cout << "    Boost compile:      " << boost_us << " us\n";
        std::cout << "    LikeMatcher build:  " << like_us << " us\n";
    }
}

TEST(PatternMatchBenchmark, StringLengthScaling) {
    using namespace milvus;
    PatternMatchTranslator translator;
    ...
    std::vector<size_t> lengths = {10, 50, 200, 1000, 5000};

    std::cout << "\n====== String Length Scaling (pattern: %abc%def%, 50000 "
                 "strings) ======\n";

    for (size_t len : lengths) {
        auto data = GenerateRandomStrings(50000, len);
        ...
        results.push_back(RunBench(
            "RE2", data, [&](const std::string& s) { return re2_matcher(s); }));
        results.push_back(RunBench("Boost", data, [&](const std::string& s) {
            return boost_matcher(s);
        }));
        results.push_back(
            RunBench("LikePatternMatcher", data, [&](const std::string& s) {
        ...
        PrintResults("avg_len=" + std::to_string(len), results);
    }
}

TEST(PatternMatchBenchmark, RealisticData) {
    using namespace milvus;

    // Generate realistic-looking data (paths, names, IDs, emails)
    const size_t N = 100000;
    std::mt19937 rng(42);
    std::vector<std::string> data;
    data.reserve(N);

    std::vector<std::string> prefixes = {"user_",
                                         "file_",
                                         "/path/to/",
                                         "test_",
                                         "data_",
                                         ...
                                         "backup_",
                                         "report_"};
    std::vector<std::string> suffixes = {".txt",
                                         ".json",
                                         ".cpp",
                                         ".go",
                                         ".py",
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        switch (rng() % 5) {
            case 0:  // path-like
                s = prefixes[rng() % prefixes.size()] +
                    words[rng() % words.size()] + "_" +
                    std::to_string(rng() % 10000) +
                    suffixes[rng() % suffixes.size()];
                break;
            case 1:  // sentence-like
                s = words[rng() % words.size()] + " " +
                    words[rng() % words.size()] + " " +
                    words[rng() % words.size()];
                break;
            case 2:  // id-like
                s = "id_" + std::to_string(rng() % 100000);
                break;
            case 3:  // email-like
                s = words[rng() % words.size()] + std::to_string(rng() % 1000) +
                    "@" + words[rng() % words.size()] + ".com";
                break;
            case 4:  // mixed
                s = words[rng() % words.size()] + "_" +
                    words[rng() % words.size()] + "_" +
                    std::to_string(rng() % 10000);
                break;
        }
    ...
    std::vector<PatternCase> patterns = {
        {"high-match: %test%", "%test%"},
        {"low-match: %admin%query%", "%admin%query%"},
        {"path prefix: /path/to/%", "/path/to/%"},
        {"extension: %.cpp", "%.cpp"},
        {"email: %@%.com", "%@%.com"},
        {"id range: id\\_1%", "id\\_1%"},
        {"exact word: hello", "hello"},
        {"complex: %user%test%data%", "%user%test%data%"},
    };

    std::cout << "\n====== Realistic Data Benchmark (" << N
              << " strings) ======\n";
...
TEST(PatternMatchBenchmark, UnicodeData) {
    using namespace milvus;

    // Generate data with mixed ASCII and UTF-8 content
    const size_t N = 50000;
    std::mt19937 rng(42);
    std::vector<std::string> data;
    data.reserve(N);

    std::vector<std::string> utf8_words = {
        "\xE4\xBD\xA0\xE5\xA5\xBD",  // 你好
    ...
    std::vector<std::string> ascii_words = {
        "hello", "world", "test", "data", "user"};
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        for (int p = 0; p < parts; p++) {
            if (p > 0)
                s += " ";
            if (rng() % 3 == 0)
                s += utf8_words[rng() % utf8_words.size()];
            else
                s += ascii_words[rng() % ascii_words.size()];
        }
        data.push_back(std::move(s));
    }
    ...
    std::vector<PatternCase> patterns = {
        {"Chinese: %\xE4\xBD\xA0\xE5\xA5\xBD%", "%\xE4\xBD\xA0\xE5\xA5\xBD%"},
        {"Accent: %caf\xC3\xA9%", "%caf\xC3\xA9%"},
        {"Emoji: %\xF0\x9F\x98\x80%", "%\xF0\x9F\x98\x80%"},
        {"Mixed: %hello%\xE4\xB8\x96\xE7\x95\x8C%",
         "%hello%\xE4\xB8\x96\xE7\x95\x8C%"},
        {"Underscore UTF-8: caf_", "caf_"},
        {"ASCII in UTF-8 data: %test%", "%test%"},
    };

    std::cout << "\n====== Unicode Data Benchmark (" << N
              << " strings) ======\n";
