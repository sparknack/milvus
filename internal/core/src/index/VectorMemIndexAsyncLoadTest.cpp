...
class VectorMemIndexAsyncLoadTest : public ::testing::Test {
 protected:
    void
    SetUp() override {
        old_write_mode_ = storage::FileWriter::GetMode();
    ...
    void
    TearDown() override {
        storage::FileWriter::SetMode(old_write_mode_);
        auto& admission = storage::LoadAdmissionController::GetInstance();
        const storage::LoadAdmissionRequest all{admission.CapacityBytes(), 1};
        const bool acquired =
            admission.TryAcquire(all, storage::LoadAdmissionPriority::High);
    ...
    Persist(BinarySet binary, bool sliced) {
        ...
        for (const auto& [name, data] : binary.binary_map_) {
            if (sliced && data->size > 2) {
                // Buffered tests cover arbitrary historic slices; DIRECT tests
                // keep non-tail slices aligned, like the production producer.
                const size_t part =
                    storage::FileWriter::GetMode() ==
                            storage::FileWriter::WriteMode::DIRECT
                        ? std::max<int64_t>(4096, data->size / 3 / 4096 * 4096)
                        : data->size / 3;
                const auto count =
                    storage::FileWriter::GetMode() ==
                            storage::FileWriter::WriteMode::DIRECT
                        ? (data->size + part - 1) / part
                        : 3;
                for (size_t i = 0; i < count; ++i) {
                    const auto size =
                        i + 1 == count ? data->size - i * part : part;
                    files.push_back(PersistBytes(GenSlicedFileName(name, i),
                                                 data->data.get() + i * part,
                                                 size));
                }
                meta[META].push_back({{NAME, name},
                                      {SLICE_NUM, count},
                                      {TOTAL_LEN, data->size}});
            } else {
                files.push_back(
    ...
    int old_workers_{};
    size_t old_slots_{};
    storage::FileWriter::WriteMode old_write_mode_{};
};
...
TEST_F(VectorMemIndexAsyncLoadTest, MmapDirectWritesTailBeforeHeadAndDrains) {
    storage::FileWriter::SetMode(storage::FileWriter::WriteMode::DIRECT);
    auto& admission = storage::LoadAdmissionController::GetInstance();
    admission.SetCapacityBytes(DEFAULT_FIELD_MAX_MEMORY_LIMIT);
    admission.SetCapacitySlots(3);
    storage::LocalFileIOPool::GetInstance().Configure(2);
    const auto slice = storage::DefaultStreamSliceSize();
    const std::vector<uint8_t> bytes(2 * slice + 17, 42);
    const auto path = PersistBytes("payload", bytes.data(), bytes.size());
    for (const std::string_view outcome : {"success", "cancel", "read_error"}) {
        SCOPED_TRACE(outcome);
        auto fs = std::make_shared<VectorNativeFileSystem>();
        std::vector<uint8_t> wire(source_->Size(path));
        source_->Read(path, wire.data(), wire.size());
        auto file =
            std::make_shared<test::ControlledDirectReadFile>(std::move(wire));
        fs->files.emplace(path, file);
        auto context = Context();
        context.fs = fs;
        const std::vector<std::string> files{path};
        context.legacy_index_files = folly::coro::blockingWait(
            storage::InspectLegacyIndexFilesAsync(
                files,
        ...
        file->SetAutoComplete(false);
        const auto observation = std::make_shared<FileLoadObservation>();
        auto config = ConfigFor(RegisterFileLoadNode(observation));
        test::TmpPath dir;
        const auto target = (dir.get() / "index").string();
        config[INDEX_FILES] = files;
        config[MMAP_FILE_PATH] = target;
        observation->finalize = [&](const std::string& filename) {
        ...
        VectorMemIndex<float> loaded(DataType::NONE,
                                     config.at(INDEX_TYPE).get<std::string>(),
                                     "L2",
                                     version_,
                                     true,
                                     context);
        ...
        ASSERT_TRUE(file->WaitForCallCount(3));
        const auto reads = file->DirectReadCalls();
        EXPECT_EQ(reads[0].nbytes, slice);
        EXPECT_EQ(reads[1].nbytes, slice);
        EXPECT_EQ(reads[2].nbytes, 17);
        file->Complete(2);
        // The tail must reach disk while the first two remote reads are held.
        const auto deadline =
            std::chrono::steady_clock::now() + std::chrono::seconds(5);
        while (std::filesystem::file_size(target) < bytes.size() &&
               std::chrono::steady_clock::now() < deadline) {
            std::this_thread::sleep_for(std::chrono::milliseconds(1));
        }
        ASSERT_GE(std::filesystem::file_size(target), bytes.size());
        ...
        file->Complete(
            0,
            outcome == "read_error"
                ? arrow::Status::IOError("injected head read failure")
                : arrow::Status::OK());
        if (outcome != "success") {
            EXPECT_EQ(load.wait_for(std::chrono::milliseconds(30)),
                      std::future_status::timeout);
        ...
        if (outcome == "success") {
            ...
            EXPECT_TRUE(observation->finalized);
            EXPECT_EQ(std::filesystem::file_size(target), bytes.size());
        } else {
...
TEST_F(VectorMemIndexAsyncLoadTest, MmapRejectsMultipleEntriesForOneFile) {
    const auto observation = std::make_shared<FileLoadObservation>();
    const auto type = RegisterFileLoadNode(observation);
    auto config = ConfigFor(type);
    const uint8_t byte = 42;
    config[INDEX_FILES] = std::vector<std::string>{
        PersistBytes("first", &byte, 1), PersistBytes("second", &byte, 1)};
    test::TmpPath dir;
    const auto target = (dir.get() / "index").string();
    config[MMAP_FILE_PATH] = target;
    VectorMemIndex<float> loaded(
        DataType::NONE, type, "L2", version_, true, Context());
    EXPECT_THROW(loaded.Load({}, config, nullptr), SegcoreError);
    EXPECT_FALSE(observation->finalized);
    EXPECT_FALSE(std::filesystem::exists(target));
}

TEST_F(VectorMemIndexAsyncLoadTest, MmapWriteFailureCleansTarget) {
    ...
    test::TmpPath dir;
    const auto target = (dir.get() / "index").string();
    std::filesystem::create_symlink("/dev/full", target);
    config[MMAP_FILE_PATH] = target;
    VectorMemIndex<float> loaded(
        DataType::NONE, type, "L2", version_, true, Context());
    try {
