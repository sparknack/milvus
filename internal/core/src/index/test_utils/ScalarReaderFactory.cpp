...
ReaderBackend::DeriveCaps(const BackendCaseMetadata& metadata) const {
    ...
    if (spec_.wrap_artifact) {
        params =
            AnnotateJsonProjectionCompleteness(std::move(params),
                                               storage::IndexEntryDirectory{},
                                               {{"has_non_exist", false}});
    }
...
ReaderBackend::Open(storage::ArtifactPtr artifact,
    ...
    // Serialized scalar profiles exercise V3, including metadata and CRCs.
    // Legacy format coverage uses the explicit OpenV1V2 test helper.
    ...
    const auto resolved_family =
        ResolvePackedLoadFamily(spec_.family, source->IndexMeta(), params);
    ...
    if (spec_.wrap_artifact) {
        params = AnnotateJsonProjectionCompleteness(
            std::move(params), source->Directory(), source->IndexMeta());
    }
