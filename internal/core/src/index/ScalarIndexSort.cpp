...
ScalarIndexSort<T>::LoadWithStreamingToDisk(
    ...
    // Data slices are streamed to disk in batches to avoid loading the
    // entire index into memory at once.
    ...
    // Sort data slices by slice index to ensure correct concatenation order
    ...
    // Stream data slices to local disk file in batches
    is_mmap_ = true;
    ...
    int64_t total_data_size = 0;
    {
        ...
        // Write mmap padding in case of all null values
    ...
    // Load metadata files to memory (tiny: index_length, index_num_rows, etc.)
    auto index_datas =
