...
class SortedIndexLoader final {
 public:
    static constexpr std::string_view kFamily = families::kSort;

    static ReaderCaps
    DeriveCaps(const Config& index_meta);

    // Plan V3 typed memory and file destinations without reading payloads.
    static IndexLoadPlan
    PlanPacked(const storage::IndexEntryDirectory& directory,
               const nlohmann::json& metadata,
               const storage::LoadOptions& opts);

    // Validate completed destinations and transfer them into query state.
