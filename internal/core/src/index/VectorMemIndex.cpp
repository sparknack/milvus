...
VectorMemIndex<T>::LoadFromFileAsync(const Config& config,
    ...
    std::vector<std::unique_ptr<storage::PositionedFileWriter>> writers(
        paths.size());
    ...
    try {
        // Clear all configured cache targets, including unused files on an
        // all-null reload. Their entry lengths are learned during preparation.
        co_await storage::RunLocalFileIOAsync(
            [&] {
                for (const auto& path : paths) {
                    ...
                    storage::PositionedFileWriter empty(
                        path,
                        0,
        ...
        storage::MemFileManagerImpl::IndexEntryConsumerFactory prepare =
            ...
            -> folly::coro::Task<storage::LegacyIndexConsumer> {
            if (IsValidDataBinary(name) || name == EMPTY_EMB_LIST_OFFSET_KEY) {
                ...
                co_return [destination](size_t offset,
            ...
            // Knowhere serializes one main entry; embedding-list metadata and
            // its optional raw index each have a separate file. Entry-relative
            // offsets are therefore file-relative, including the final tail.
            AssertInfo(writers[writer] == nullptr,
                       "Multiple Knowhere entries target mmap file {}: {}",
                       paths[writer],
                       name);
            co_await storage::RunLocalFileIOAsync(
                [&] {
                    storage::ThrowIfCancelled(token,
                                              "VectorMemIndex::CreateMmap");
                    writers[writer] =
                        std::make_unique<storage::PositionedFileWriter>(
                            paths[writer],
                            size,
                            storage::io::GetPriorityFromLoadPriority(priority));
                },
                priority);
            co_return [&, writer](size_t offset, std::span<const uint8_t> bytes)
                          -> folly::coro::Task<void> {
                ...
                co_await storage::RunLocalFileIOAsync(
                    [&] {
                        ...
                        const auto start_write =
                            std::chrono::steady_clock::now();
                        writers[writer]->WriteAt(
                            offset, bytes.data(), bytes.size());
                        milvus::monitor::internal_storage_write_disk_duration
                            .Observe(std::chrono::duration_cast<
