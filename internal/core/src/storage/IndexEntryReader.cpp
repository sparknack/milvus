...
struct AsyncActiveSliceTask {
    ...
    milvus::segcore::async_load::LoadBudgetLease lease;
...
struct PendingAsyncAdmission {
    ...
    size_t budget_bytes{0};
    size_t read_bytes{0};
    folly::SemiFuture<milvus::segcore::async_load::LoadBudgetLease> future;
};
...
ReadOrderedEntryStreamAsync(
    ...
    auto ensurePendingAdmission = [&]() {
        ...
        pending_admission =
            PendingAsyncAdmission{seq,
                                  budget_bytes,
                                  slice_read_bytes(seq),
                                  scheduler.Admit(priority, budget_bytes)};
    };
