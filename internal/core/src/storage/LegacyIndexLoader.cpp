...
InspectLegacyIndexFileImpl(milvus::InputStream& input,
    ...
    const auto file_bytes = input.Size();
    ...
    size_t descriptor_bytes;
    {
        ...
        co_await ReadRange(
            input, 0, prefix.get(), prefix_bytes, priority, token);
        int32_t magic;
    ...
    LegacyIndexFileInfo info;
    {
        ...
        co_await ReadRange(
            input, 0, descriptor.get(), descriptor_bytes, priority, token);
...
InspectLegacyIndexFile(const ChunkManagerPtr& chunk_manager,
    ...
    return folly::coro::blockingWait(
        InspectLegacyIndexFileImpl(input, priority, std::move(token)));
}
}  // namespace milvus::storage
