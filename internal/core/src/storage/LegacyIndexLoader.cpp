...
// A bounded raw range can coexist with a DIRECT I/O copy, including tail padding.
size_t
...
StreamLegacyIndexFilesAsync(std::span<const LegacyIndexFile> files,
    ...
    auto wait_for_completion = [&]() -> folly::coro::Task<void> {
        const auto bytes = co_await completions.dequeue();
    ...
    auto run_unit = [&](Unit unit,
                        ...
                        size_t charge) -> folly::coro::Task<void> {
        try {
        ...
        for (const auto& file : files) {
            ...
            do {
                ...
                const auto charge = info.raw_payload
                                        ...
                                        : info.max_transient_bytes;
                while (
                    inflight >= kMaxIndexLoadInflightSlices ||
                    (inflight != 0 &&
